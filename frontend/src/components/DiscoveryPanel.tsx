...
import { MapPin, SparklesIcon, XIcon } from "lucide-react";
...
interface DiscoveryPanelProps {
  ...
  aiError: string | null;
  aiQuery: string;
  reasoningMap: Map<string, string> | null;
}
    ...
    <div className="w-[320px] h-full flex flex-col bg-background border-r-[0.5px] border-border flex-shrink-0">
      <div className="p-6 flex flex-col gap-5 border-b-[0.5px] border-border bg-white z-10">
        <div className="flex items-center gap-2 text-text">
          <MapPin size={18} />
          <h1 className="text-lg font-medium">Spots near Toronto</h1>
        </div>
        ...
        <div className="flex flex-col gap-2">
          <form
            ...
            className="flex items-center gap-2 bg-background rounded-lg border-[0.5px] border-border px-3 py-2"
          >
            <SparklesIcon size={14} className="text-amber-500 flex-shrink-0" />
            <input
              type="text"
              value={searchInput}
              onChange={(e) => setSearchInput(e.target.value)}
              placeholder="Describe your ideal work spot..."
          ...
          {aiQuery && !aiLoading && !aiError && (
            <p className="text-xs text-muted">
              Showing AI picks for &ldquo;{aiQuery}&rdquo;
            </p>
          )}
        </div>

        {/* Filters — hidden during AI mode */}
        {!aiQuery && (
          ...
          spots.map((spot) => (
            <div key={spot.id} id={`spot-${spot.id}`}>
              <SpotCard
                ...
                aiReasoning={reasoningMap?.get(spot.id)}
              />
            </div>
          ))
        )}
